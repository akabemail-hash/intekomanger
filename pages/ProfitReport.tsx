...
export const ProfitReport: React.FC = () => {
  ...
  const [profitKey, setProfitKey] = useState<string>('');
  ...
  const handleSearch = async () => {
      ...
      try {
          ...
          if (data && data.length > 0) {
              ...
              const gKey = findKey(firstItem, ['CategoryName', 'categoryName', 'Group', 'group', 'qrup']);
              const pKey = findKey(firstItem, ['ProductName', 'productName', 'malinadi', 'Name']);
              const profitK = findKey(firstItem, ['ProfitAmount', 'profitAmount', 'Profit']);

              if (sKey) setSupplierKey(sKey);
              if (gKey) setGroupKey(gKey);
              if (pKey) setProductKey(pKey);
              if (profitK) setProfitKey(profitK);
              ...
              if (gKey) {
                  setUniqueGroups(Array.from(new Set(data.map((i: any) => i[gKey]))).filter(Boolean).sort() as string[]);
              }
              if (pKey) {
  ...
  useEffect(() => {
      ...
      setFilteredData(res);
  }, [rawData, selectedSupplier, selectedGroup, selectedProduct, supplierKey, groupKey, productKey]);
  ...
  const totalProfit = profitKey 
    ? filteredData.reduce((acc, curr) => acc + (Number(curr[profitKey]) || 0), 0) 
    : 0;
  ...
  const generatePdfContent = async (doc: jsPDF) => {
    ...
    doc.setFontSize(14); doc.text(t('profit_report'), 14, 30);
  ...
  const handleExcelExport = async () => {
    ...
    worksheet.getCell('A4').value = t('profit_report');
    ...
    a.href = url; a.download = `profit_report_${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{t('profit_report')}</h1>
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
            ...
            {/* Dynamic Filters (Supplier, Group, Product) */}
            <div>
      ...
      {!hasSearched ? (
           <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-12 text-center">
               ...
               <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">{t('profit_report')}</h3>
        ...
        <>
            {profitKey && (
                <div className="bg-emerald-600 text-white p-4 rounded-xl shadow-md flex justify-between items-center animate-in fade-in slide-in-from-top-4">
                    <span className="font-medium text-emerald-100">{t('total')} {t('profitamount')}:</span>
