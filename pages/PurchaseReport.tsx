...
export const PurchaseReport: React.FC = () => {
  ...
  const [totalKey, setTotalKey] = useState<string>(''); // PayableAmount
  ...
  const handleSearch = async () => {
      ...
      try {
          ...
          if (data && data.length > 0) {
              ...
              const tKey = findKey(firstItem, ['PayableAmount', 'payableAmount', 'payableamount', 'Total', 'total']);
  ...
  const translateHeader = (header: string) => {
      ...
      // Special check: if column is "Date", map to "purchasedate" for this report
      if (key === 'date') key = 'purchasedate';
  ...
  const generatePdfContent = async (doc: jsPDF) => {
    ...
    doc.setFontSize(14); doc.text(t('purchase_report'), 14, 30);
    ...
    autoTable(doc, {
        ...
        headStyles: { fillColor: [16, 185, 129], font: 'Roboto' } // Emerald for purchase
    });
  ...
  const handleExcelExport = async () => {
    ...
    worksheet.getCell('A4').value = t('purchase_report');
    ...
    a.href = url; a.download = `purchase_report_${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">{t('purchase_report')}</h1>
      ...
      {!hasSearched ? (
           <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-12 text-center">
               ...
               <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">{t('purchase_report')}</h3>
        ...
        <>
            {totalKey && (
                <div className="bg-emerald-600 text-white p-4 rounded-xl shadow-md flex justify-between items-center animate-in fade-in slide-in-from-top-4">
                    <span className="font-medium text-emerald-100">{t('total')}:</span>
