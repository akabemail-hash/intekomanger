...
import { Download, FileSpreadsheet, Search, ChevronLeft, ChevronRight, AlertCircle, RefreshCcw } from 'lucide-react';
...
export const SaleRefundReport: React.FC = () => {
  ...
  const [refundAmountKey, setRefundAmountKey] = useState<string>('');
  ...
  const handleSearch = async () => {
    ...
    //      console.log('Fetched refund data:', data);
          ...
          if (data && data.length > 0) {
              ...
              const rAmtKey = findKey(firstItem, ['RefundAmount', 'refundAmount', 'refundamount']);

              if (sKey) setSupplierKey(sKey);
              if (pKey) setProductKey(pKey);
              if (rAmtKey) setRefundAmountKey(rAmtKey);
          ...
          console.error("Failed to fetch refund report", e);
  ...
  useEffect(() => {
      ...
      setFilteredData(res);
  }, [rawData, selectedSupplier, selectedProduct, supplierKey, productKey]);
  ...
  const totalRefundAmount = refundAmountKey 
    ? filteredData.reduce((acc, curr) => acc + (Number(curr[refundAmountKey]) || 0), 0) 
    : 0;
      ...
      if (header.toLowerCase().includes('date') || header.toLowerCase() === 'tarix') {
          try {
             if (typeof value === 'string' && value.includes('T')) {
                 ...
                 if (!isNaN(d.getTime())) {
                      return d.toLocaleDateString(language === 'en' ? 'en-US' : (language === 'tr' ? 'tr-TR' : (language === 'ru' ? 'ru-RU' : 'az-AZ'))) + ' ' + d.toLocaleTimeString([], {hour:'2-digit', minute:'2-digit'});
                 }
             }
  ...
  const generatePdfContent = async (doc: jsPDF) => {
    ...
    doc.setFontSize(14); doc.text(t('sale_refund_report'), 14, 30);
    ...
    autoTable(doc, {
        ...
        headStyles: { fillColor: [16, 185, 129], font: 'Roboto' } // Emerald green
    });

    doc.save(`sale_refund_report_${new Date().toISOString().split('T')[0]}.pdf`);
  };

  const handleExcelExport = async () => {
    if (filteredData.length === 0) return;
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Sale Refund Report');
    ...
    worksheet.getCell('A4').value = t('sale_refund_report');
    ...
    a.href = url; a.download = `sale_refund_report_${new Date().toISOString().split('T')[0]}.xlsx`;
    a.click();
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
            ...
            {/* Dynamic Filters (Supplier, Product) */}
            <div>
              ...
              <select 
                ...
                disabled={!hasSearched || rawData.length === 0 || !supplierKey}
              >
      ...
      {!hasSearched ? (
           <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-100 dark:border-slate-700 p-12 text-center">
               ...
               <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">{t('sale_refund_report')}</h3>
        ...
        <>
            {refundAmountKey && (
                <div className="bg-emerald-600 text-white p-4 rounded-xl shadow-md flex justify-between items-center animate-in fade-in slide-in-from-top-4">
                    <span className="font-medium text-emerald-100">{t('total')} {t('refundamount')}:</span>
                    <span className="text-2xl font-bold">₼{totalRefundAmount.toLocaleString()}</span>
                </div>
            )}
